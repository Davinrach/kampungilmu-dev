...
import { sellerService, UpdateBookPayload } from "@/services/sellerService";
...
export default function EditBookPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch book detail
        const book = await sellerService.getBookDetail(bookId);
        if (!book) {
          setNotFound(true);
          return;
        }
        ...
        setFormData({
          ...
          book_type: book.book_type || "used",
          condition_grade: book.condition_grade || "layak_baca",
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await sellerService.updateBook(bookId, payload);

      // Update photos if changed
      const photosChanged = JSON.stringify(photos) !== JSON.stringify(originalPhotos);
      if (photosChanged && photos.length > 0) {
        await sellerService.updateBookPhotos(bookId, photos);
      }
  ...
  if (notFound) {
    return (
      <div className="text-center py-16">
        ...
        <h2 className="text-xl font-bold text-gray-900 mb-2">Buku Tidak Ditemukan</h2>
        <p className="text-gray-500 mb-4">Buku yang Anda cari tidak ada atau sudah dihapus</p>
        <Link
          href="/seller/books"
          className="inline-flex items-center gap-2 px-4 py-2 bg-teal-500 text-white rounded-xl font-semibold hover:bg-teal-600 transition"
        >
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-500">Perbarui informasi buku Anda</p>
      </div>
