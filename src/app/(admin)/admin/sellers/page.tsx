...
import { adminService, PendingSeller, formatDateTime } from "@/services/adminService";
...
export default function AdminSellersPage() {
  ...
  const handleApprove = async (userId: string) => {
    ...
    try {
      ...
      setPendingSellers((prev) => prev.filter((s) => s.user_id !== userId));
    } catch (err: any) {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      setPendingSellers((prev) => prev.filter((s) => s.user_id !== selectedSeller.user_id));
      setShowRejectModal(false);
      setSelectedSeller(null);
      setRejectReason("");
    } catch (err: any) {
      alert(err.response?.data?.message || "Gagal menolak seller");
    } finally {
      setProcessingId(null);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-bold text-yellow-800">{pendingSellers.length} Pengajuan Menunggu</p>
      ...
      {pendingSellers.length === 0 ? (
        ...
        <div className="space-y-4">
          {pendingSellers.map((seller) => (
            <div key={seller.user_id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
              {/* Header */}
              ...
              <div className="p-5 grid md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-2">
                    {seller.ktp_photo ? (
                      <a
                        href={seller.ktp_photo}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 px-3 py-2 bg-blue-50 text-blue-700 border border-blue-200 rounded-lg text-sm font-medium hover:bg-blue-100 transition"
                      >
                      ...
                      <span className="text-xs text-gray-400">KTP tidak tersedia</span>
                    )}
                    {seller.shop_photo ? (
                      <a
                        href={seller.shop_photo}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 px-3 py-2 bg-purple-50 text-purple-700 border border-purple-200 rounded-lg text-sm font-medium hover:bg-purple-100 transition"
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                        </svg>
                      ...
                      <span className="text-xs text-gray-400">Foto toko tidak tersedia</span>
                    )}
                  </div>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-md w-full">
            ...
            <div className="p-6 border-t border-gray-100 flex gap-3">
              <button
                onClick={() => { setShowRejectModal(false); setSelectedSeller(null); setRejectReason(""); }}
      ...
      {/* Photo Lightbox */}
      {selectedPhoto && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-[60] p-4" onClick={() => setSelectedPhoto(null)}>
          <button onClick={() => setSelectedPhoto(null)} className="absolute top-4 right-4 text-white hover:text-gray-300">
